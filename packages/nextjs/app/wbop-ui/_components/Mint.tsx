  ...
  return (
    <div className="grid justify-center items-center bg-black relative pb-10">
      <h1 className="text-center mb-8">
        <span className="block text-2xl mb-2">You have entered</span>
        <span className="block text-4xl font-bold">The</span>
        <span className="block text-4xl font-bold">Wrapped BOP MINT</span>
        WBOP can only be minted using BOP.
      </h1>
      ...
      <div className="flex flex-col justify-center w-full mx-5 sm:mx-8 2xl:mx-20">
        <div className={`mt-10 flex gap-2 ${visible ? "" : "invisible"} max-w-2xl`}>
          <div className="flex gap-5 bg-white bg-opacity-80 z-0 p-7 rounded-2xl shadow-lg">
            ...
            <div>
              <div className="text-black">Welcome to the WBOP MINT</div>
              <div className="mt-2 text-black">
                Set a spending cap for your BOP or WBOP above{" "}
                <div>
                  {" "}
                  <code className="italic bg-white text-base font-bold [word-spacing:-0.5rem]">
                    We -recommend -setting -a -cap- below- your -total -balance, but above what you plan to use!
                  </code>{" "}
                </div>
                then use your BOP to mint WBOP or swap between the assets.
              </div>
            </div>
          </div>
          <button
            className="btn btn-circle btn-ghost h-6 w-6 bg-white bg-opacity-80 z-0 min-h-0 drop-shadow-md"
        ...
        <div className="flex  relative pb-10">
          <div className="flex flex-col w-md mx-5 sm:mx-8 2xl:mx-20">
            <div className="flex flex-col  mt-6 px-7 py-8 bg-white opacity-100 rounded-2xl shadow-lg border-2 border-primary">
              <span className="text-4xl sm:text-6xl text-black">Mint WBOP</span>
                ...
                <div className="flex rounded-full border border-primary p-1 flex-shrink-0">
                  <div className="flex rounded-full border-2 border-primary p-1">
                    <button
                      ...
                      onClick={async () => {
                        try {
                          await writeContractAsync({ functionName: "mint", args: [BigInt(daiAmount)] });
                        } catch (err) {
